...
treat_results <- topTable(fit2, coef = "treatment", adjust = "BH", num = 200, p.value = .05)
head(treat_results)

treat_results$ID = row.names(treat_results)
treat_results$SYMBOL <- lapply(treat_results$ID, function(x) mget(x, env = hgu133plus2SYMBOL, ifnotfound = NA)[[1]])
head(treat_results)
...
ggplot(county.withdrawal.stays, aes(x=Time.Period, y=Rate.of.Neonatal.Hospital.Stays.with.Withdrawal.Symptoms)) + 
  geom_boxplot(outlier.colour="red", fill='#238A8DFF', alpha = .6) +  geom_jitter(shape=16, alpha = .3,    position=position_jitter(0.2)) + 
   ggtitle("Rate of NAS Withdrawal Symptoms in 2016 vs 2017") + 
...
county <-county %>% 
  ...
  mutate(County.FIPS.Code.x = sprintf("%03d", County.FIPS.Code.x)) %>%
  rename(County.FIPS.Code = County.FIPS.Code.x)
...
### Philadelphia data -- who does this affect in Philadelphia? 
Graph of rate of hospital stays
``` {r eval = TRUE}
#USE Q date not annual and see over time for Philadelphia and then all counties 

#compare how all CSVs work together 
```
